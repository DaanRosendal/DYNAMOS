...
func handleSqlDataRequest(ctx context.Context, msComm *pb.MicroserviceCommunication) error {
	_, span := trace.StartSpan(ctx, "handleSqlDataRequest")
	defer span.End()
	...
	// print hello world as log
	logger.Sugar().Debugf("Hello world from %s", serviceName)
	msComm.Result = []byte("Hello world from " + serviceName)
